...
namespace VulkanModule {
    ...
    void Vulkan::CleanupOldMeshes() {
        for (const auto& mesh : root->assetManager.loadedMeshes) {
            for (const auto& submesh : mesh.second.submeshes) {
                vkDestroyBuffer(vDevice.device, submesh.indexBuffer, nullptr);
                vkFreeMemory(vDevice.device, submesh.indexBufferMemory, nullptr);
                vkDestroyBuffer(vDevice.device, submesh.vertexBuffer, nullptr);
                vkFreeMemory(vDevice.device, submesh.vertexBufferMemory, nullptr);
            }
        }
    }
}