...
namespace VulkanModule {
    ...
    void VulkanBuffer::CreateVertexBuffer(Data::Mesh* mesh) {
        for (auto& submesh : mesh->submeshes) {
            ...
            VkDeviceSize bufferSize = sizeof(submesh.vertices[0]) * submesh.vertices.size();
            ...
            CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, submesh.vertexBuffer, submesh.vertexBufferMemory);

            CopyBuffer(stagingBuffer, submesh.vertexBuffer, bufferSize);
    ...
    void VulkanBuffer::CreateIndexBuffer(Data::Mesh* mesh) {
        for (auto& submesh : mesh->submeshes) {
            VkDeviceSize bufferSize = sizeof(submesh.indices[0]) * submesh.indices.size();
            ...
            // Staging buffer to vertex buffer
            CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, submesh.indexBuffer, submesh.indexBufferMemory);

            CopyBuffer(stagingBuffer, submesh.indexBuffer, bufferSize);
