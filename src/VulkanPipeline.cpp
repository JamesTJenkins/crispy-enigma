...
namespace VulkanModule {
    VulkanPipeline::VulkanPipeline(Root* _root, VulkanDevice* device, VulkanSwapchain* swapchain, VulkanRenderPass* renderpass, VulkanDescriptor* descriptor) : root(_root), vDevice(device), vSwapchain(swapchain), vRenderpass(renderpass), vDescriptor(descriptor) {
    ...
    void VulkanPipeline::CreateGraphicsPipeline(std::string shaderName){
		Data::Shader* shader = &root->assetManager.loadedShaders[shaderName];
		// Create all shader infomation and load into stages

		CreateGraphicsPipeline(shader);
	}

	void VulkanPipeline::CreateGraphicsPipeline(Data::Shader* shader){
		// Create all shader infomation and load into stages
		...
		multisampling.rasterizationSamples = vDevice->msaaSamples;
		...
		pipelineLayoutInfo.pSetLayouts = &vDescriptor->descriptorSetLayout;
		pipelineLayoutInfo.pPushConstantRanges = &push_constant;
		pipelineLayoutInfo.pushConstantRangeCount = 1;

		if (vkCreatePipelineLayout(vDevice->device, &pipelineLayoutInfo, nullptr, &shader->pipelineLayout) != VK_SUCCESS) {
	...
	void VulkanPipeline::CreateAllGraphicPipelines() {
		for (auto& shader : root->assetManager.loadedShaders){
			CreateGraphicsPipeline(shader.first);
		}
	}
	
	void VulkanPipeline::Cleanup() {
		for (auto& shader : root->assetManager.loadedShaders){
			vkDestroyPipeline(vDevice->device, shader.second.graphicsPipeline, nullptr);
			vkDestroyPipelineLayout(vDevice->device, shader.second.pipelineLayout, nullptr);
		}
	}
}